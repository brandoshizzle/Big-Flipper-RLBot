...
class Vec3:
    ...
    def __init__(
        self, x: Union[float, "Vec3", "Vector3"] = 0, y: float = 0, z: float = 0
    ):
        """
    ...
    def line_to(self, other: "Vec3") -> "Vec3":
        """Returns a new vector line between two points."""
        return Vec3(other.x - self.x, other.y - self.y, other.z - self.z).normalized()

    def ang_to(self, ideal: "Vec3") -> float:
        """Returns the angle to the ideal vector. Angle will be between 0 and 180."""
        cos_ang = self.dot(ideal) / (self.length() * ideal.length())
        return math.degrees(math.acos(cos_ang))

    def ang_to_flat(self, ideal: "Vec3") -> float:
        """Returns the angle to the ideal vector. Angle will be between 0 and 180."""
        ideal.z = 0
        cos_ang = self.dot(ideal) / (self.length() * ideal.length())
        return math.degrees(math.acos(cos_ang))
